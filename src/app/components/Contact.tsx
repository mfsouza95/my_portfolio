...
export default function Contact() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch('https://formspree.io/f/mnjybqoo', {
        method: 'POST',
    ...
    return (
      <section className="py-20 max-w-4xl mx-auto px-4">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 md:p-10 shadow-2xl backdrop-blur-md text-center space-y-4">
          <p className="text-teal-400 font-mono text-sm uppercase tracking-widest">// message sent</p>
          <p className="text-white text-lg font-semibold">Thanks for reaching out!</p>
          <p className="text-slate-400 text-sm">I'll get back to you as soon as possible.</p>
          <button
            onClick={() => setStatus('idle')}
            className="mt-4 font-mono text-xs uppercase tracking-wider px-5 py-2.5 rounded-md border border-white/10 bg-white/5 hover:border-blue-500/50 hover:bg-blue-500/5 transition-all text-slate-300"
          >
    ...
    <section className="py-20 max-w-4xl mx-auto px-4">
      ...
      >
        ...
        <div className="flex justify-end pt-2">
          <button
            type="submit"
            disabled={status === 'loading' || (mounted && !turnstileToken)}
