...
export default function Sidebar() {
  ...
  return (
    <>
      {state.isOpen && (
        <div
          className="fixed inset-0 z-40 bg-black/40 backdrop-blur-sm overlay-animation"
          onClick={() => dispatch({ type: 'CLOSE_SIDEBAR' })}
        />
      )}

      <aside
        className={`fixed top-0 right-0 h-full w-80 z-50 bg-[#020617] border-l border-white/10 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${
                ...
                <div key={productId} className="flex items-start gap-4 bg-white/5 border border-white/10 rounded-xl p-4 relative">
                  ...
                  <div className="flex flex-col gap-3 flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <button
                        onClick={() => dispatch({ type: 'CLEAR_ITEM', productId })}
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-lg px-2 py-1">
                        <button
                          onClick={() => dispatch({ type: 'DECREASE', productId })}
                        ...
                        <button
                          onClick={() => dispatch({ type: 'INCREASE', productId })}
                          className="text-slate-400 hover:text-white transition-colors"
        ...
        {totalQuantity > 0 && (
          <div className="px-6 py-5 border-t border-white/10 flex flex-col gap-3">
            <p className="text-slate-500 font-mono text-[10px] leading-relaxed text-center">
              // pressing <span className="text-teal-400">Buy now</span> will download your {cartItems.length > 1 ? 'CVs' : 'CV'}
            </p>
            <button
              onClick={handleBuyNow}
              className="flex items-center justify-center gap-2 w-full bg-linear-to-r from-teal-500 to-blue-500 text-white font-mono font-medium text-xs uppercase tracking-wider px-6 py-3 rounded-lg hover:opacity-90 active:scale-95 transition-all shadow-lg"
            >
              <FileDown size={14} />
              Buy now
            </button>
            <button
              ...
              className="flex items-center justify-center gap-2 w-full font-mono text-xs uppercase tracking-wider px-6 py-3 rounded-lg border border-white/10 bg-white/5 hover:border-red-500/40 hover:bg-red-500/5 hover:text-red-400 text-slate-400 transition-all"
            >
              <Trash2 size={13} />
              Empty cart
            </button>
